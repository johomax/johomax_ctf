...
def main():
    ...
    a = ap.parse_args()
    rounds = collections.defaultdict(lambda: collections.defaultdict(lambda: {"eps": 0, "seats": 0, "score": 0.0, "wins": 0, "kills": 0, "tk": 0, "best": 0.0}))
    ...
    for f in glob.glob("research/br_rounds/*/*.json"):
        ...
        for p in parts:
            ...
            if p.get("policy_name") == a.name:
                row = rounds[rn][label]; row["seats"] += 1; row["score"] += sc or 0; row["best"] = max(row["best"], sc or 0)
    ...
    for rn in sorted(rounds):
        for label, row in rounds[rn].items():
            ...
            print(f"round {rn} {label:>28} eps {row['eps']:2d} seats {row['seats']:2d} mean {row['score']/n:6.1f} best {row['best']:5.0f} wins {row['wins']//2:2d} kills {row['kills']:2d} tk {row['tk']:2d} | top others: {top_s}")

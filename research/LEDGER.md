...
- 2026-09-11 20:19Z RESUME (44 h after the stop). *** RANK 1 ***: Jordan 14,255; docxology 10,659; daveey-1 9,563; Andre von Auto 9,283; relh 8,425 — the standings converged to the v17 cap steady state and v163 (cap hunter + pact) leads. v163 still ACTIVE (lpm_6b1e63f…). Engine: latest tag paintbot-v0.7.394 = 1a8d3695 (09-11 20:02Z); 28 commits since 1fbb6b83 incl. #529 'S7 cap-ceiling sweep on real GV62 play — 2^21 reported clears both bounds; the ceiling is the lever' (a cap change may be coming), #525 realized-economy stamp + per-seat glory-by-deed on the wire, #544 shell planner perf. Fetching rounds 4645+ (background), watchers restarted.
- 20:22Z S7 ship note (docs/designs/glory/CAP-CEILING-S7.md): the 2^21 reported ceiling arms via a manifest flip + GLORYVERSION 17→18 + GameVersion 63→64 on battle-royale-s2 — not shipped at 0.7.394 (GameVersion 62). Watch coworld_version/GameVersion per round; when GLORYVERSION 18 lands, legs up to 2,097,152 return and wins/heat chains matter again. Forum quiet since 09-09 21:32 (Solbiati: 'R4611 ladder rescale is real; half of every seat row now pays 1').
- 20:25Z v163 FULL WINDOW rounds 4645-4820 (172 rounds, 2,174 eps): 99 caps = 0.55 per 12 — FIELD BEST (daveey 0.50, relh 0.50, Andre von Auto 0.49, docxology 0.46, daveey-1 0.45, Lawrence 0.44, soft-codexter 0.42, softmaxclaudius 0.37); K/ep 0.85, wins 3.1%; cap still 16,384 through coworld 0.7.392 (GameVersion 62). Cap rate is stable across 50-round blocks. Plan: local cap-rate screening (the local engine caps identically; the >=10k column of s2_seat_glory = caps) of hunter variants vs v195, 4 parallel batches x 16 eps per arm.
- 20:31Z LOCAL CAP SCREENS (engine v44, GameVersion 62, seeds 1500-1515, 4 of our seats vs 12 starters; caps = the >=10k column of s2_seat_glory): v195 baseline (port 9370), v196 hunter lane_warden base (margin 240, standoff 300-600, hpFloor 1; 9371), v197 jackal earshot 900 (9372), v198 open hunt edge_ride 240/160/0.5 instead of the 420 shelter (9373). Need ~250+ seats per arm; first batches give 64.
- 21:18Z local cap screens, batch 1 (64 seats each): v195 2 caps (K 0.64, win 5%); v196 hunter lane 0 caps (K 0.64) → dropped; v197 jackal 900 1 cap (K 0.84, win 6%); v198 open hunt 1 cap (K 0.73, win 7%). Too few caps to rank; extending v195/v197/v198 to 256 seats each. Standing 21:04Z: Jordan 14,624 rank 1 (Andre von Auto 10,279).
- 21:55Z local cap screens final (192 seats per arm, starters field): v195 7 caps = 0.44 per 12 (K 0.69, win 5.0%); v197 jackal 900 3 = 0.19 (K 0.74, win 5.7%); v198 open hunt 2 = 0.13 (K 0.70, win 7.6%); v196 hunter lane 0/64. v195 (live) stays. HOSTED JUMP at 21:49Z: Jordan 102,438 (rank 1), Lawrence 49,761, daveey 49,534, Aaron 48,839, softmaxclaudius 44,461 — standings x5 in one tick → economy/ceiling change suspected; checking versions.
- 21:58Z *** GLORYVERSION 18 LIVE *** from coworld 0.7.397 = round 4828 (~21:20Z 09-11): engine #538 'S8 SHIP — ceiling 2^21 + tier-completion bonus ON + monotonic tiers + PLACEMENT LADDER B (scoring only; GameVersion stays 63)'; latest tag 0.7.399 = 9aff25f8. Reported cap now 2,097,152; rounds 4828-4832 show 1-3 legs above 16,384 per round, max 2,097,152. OUR round 4828 sum 2,097,642 = one capped 2.1M leg → standing 102,438 (rank 1; Lawrence 49.8k, daveey 49.5k, Aaron 48.8k). Local screens under the old cap are now moot; rebuilding the engine clone at 0.7.399 (/private/tmp/engine-main-v45, server /tmp/johomax-ctf-server-runtime9).
- 22:02Z GV18 DETAILS (wiki glory-season-2, S8): ceiling 2,097,152 reported (binds ~0.3% of seat-episodes); Tier IV claim x2.50 (was x1.05), Tier V non-first x3.00 (x2.00), Tier V first x3.46; tier-completion bonus x2/x3/x4 on a Tier V claim when the same team already banked 2/3/4 lower tiers of that tree; placement FINAL 8/4/2 x1.15/x1.30/x1.60 (all below the x2 pop/gate line); everything else as GV17 (tag x2.2, heat x1/5/14/36, first blood x4, longshot x6, win x8 at finalize). First GV18 capped legs (2,097,152): Jordan 3 kills LOSS (4828), richard 3K loss, Lawrence 2K loss, Aaron 2K loss, daveey 3K loss x2, softmaxclaudius 5K win, soft-codexter 6K loss — fast deed chains still cap without winning; a win is x8 below the cap only. Metric now: analysis/s2_caps.py reports capped (>=2,097,152) and big (>=16,384) legs per 12.
- 22:00Z engine clone /private/tmp/engine-main-v45 at origin/main (0.7.399, GameVersion 63, GloryVersion 18) built: server /tmp/johomax-ctf-server-runtime9; local config v4 = v3 + achievementLightableModes (research/s2_patches/local-config-solo-v4.json). Local GV18 baseline of v195 running (port 9380). Codex GV18 analysis running (task-mtxhxvua-p38bt9).
- 22:20Z GV18 REPORT (research/s2_glory_v18.md): the percent-fold precheck P >= floor(C/q) still clamps ~100x early at the new C = 2^31, so caps need only 2-3 fast deeds: A) tag+First Blood then two heated tags (3 kills); B) first longshot + First Blood + Gun-V + one heated deed (2 kills); C) tag+First Blood then longshot+Gun-V (2 kills); D) repeated longshots (3); E) delayed first longshot after 3 gun kills (tier-completion bonus); F/G spray double-splash / grenade double-blast; H) pact-stacked Bounty-longshot First Blood (1 kill). Wins cannot be the first cap trigger (FINAL 2 folds first); after GLORY_CAP_HIT nothing raises the bank, dying is free, only friendly-fire halves it (2,097,152 → 1,048,576 → 524,288). Ladder: one cap adds ~104,858 to the standing; target ≥0.5 caps per 12; first 5 GV18 rounds 9 caps in 1,200 field seats (0.09 per 12). Ranked one-variable tests: (1) recalled enterLead 80→160 (+0.02-0.08), (2) append bounty THIRD after weakened,isolated (0 to +0.05), (3) a corrected weak-target lane_warden rung below jackal/loot (guard: alive, enemy_count==1, nearest 500-1300, weakest_enemy_hp 1-2, in_zone, zone_ticks_until_outside>120, no grenade/spray; params 240/866/1100/260/2). Keep jackal 700, loot, supply_run, pact. Standing 22:04Z: Aaron 148,954 (capped leg) rank 1; Jordan 93,414 rank 2. DECISION: ship v199 = v195 + recalled enterLead 160; v200 (bounty third) next.
- 22:23Z v199 local check on engine v45: 24 calls accepted, 0 rejected (the chained check had grepped a starter's log — slot assignment varies per seed; check via assign.json). Shipping v199 now.
- 22:26Z local GV18 baseline v195 (64 seats, starters): max 86,110, 1 big leg, K 0.69, win 9% — no local cap; local screens cannot resolve GV18 cap rates. v199 uploaded as jordan-ctf-candidate:v164, submitted, qualifying.
- 22:29Z jordan-ctf-candidate:v164 (= v199: v195 + recalled enterLead 160) ACTIVE; v163 benched after rounds 4645-4834 (GV17: 0.55 caps/12 field-best; GV18: 1 capped leg in ~6 rounds). 24-round window from ~4835.
- 22:29Z v201 (v195 + corrected weak-target lane_warden rung below jackal/loot; guard alive, enemy_count==1, nearest 500-1300, weakest_enemy_hp 1-2, in_zone, zone_ticks_until_outside>120, no grenade/spray) accepted locally: 24 calls, 0 rejected. Queue after v164's window: v200 (bounty third) → v201.
- 22:35Z pre-uploaded (unsubmitted): jordan-ctf-candidate:v165 = v200 bounty third, jordan-ctf-candidate:v166 = v201 weak-target lane.
- 22:34Z 2026-09-11 STOP (user request). All watchers, loops, batches and Codex jobs stopped. v164 (= v199: v195 + recalled enterLead 160) ACTIVE since 22:29Z, first round ~4836. Rounds 4833-4835 (v163): sums 164 / 458 / 14k, 0 caps. Standing 22:33Z: Lawrence 143,708; Aaron 127,851; Jordan 80,860 (rank 3); gloriouslyagentic 49,535; Games Bond 39,235. GV18 cap rates since 4828 (77 eps each): daveey 0.32 per 12, Aaron 0.31, everyone else incl. Jordan 0.16. RESUME: (1) restart watchers (leaderboard python3 /tmp/lb_tick.py; rounds br_rounds.py fetch --since 4836 + s2_caps.py --since 4836; push loop); (2) judge v164 after ≥24 rounds by capped legs per 12 vs the field; then submit v165 (= v200 bounty third) and v166 (= v201 weak-target lane), each ≥24 rounds; (3) check engine tags/GloryVersion before any ship (S8 = GLORYVERSION 18 at 0.7.397; a fix of the cap precheck bug would lengthen cap routes); (4) bigger levers if the queue is level: a custom heat-chain hunter play (pursue the nearest fresh enemy for 270 ticks after our own kill), first-blood aggression at spawn, spray/grenade-seeking loot. Reports: research/s2_glory_v18.md, s2_v17_cap_routes.md, s2_v161_window.md.
